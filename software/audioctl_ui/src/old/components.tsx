...
export const Send = (props: {
  ...
  (
    <div
      className="send"
      dataset={{ from: props.input.name, to: props.output.name }}
    >
...
export const Input = (props: {
  ...
  const sends = props.input.sends
    .entries()
    .map(([i, _]) => (
      <Send
        input={props.input}
        output={props.state.buses[i]}
        onMute={props.onSendMute(i)}
        onVolume={props.onSendVolume(i)}
      />
    ));

  return (
    <div className="channel" dataset={{ name: props.input.name }}>

...
export class MixerUI {
  ...
  public toggleSetupMode(setup: boolean) {
    for (const gainSlider of document.querySelectorAll<HTMLDivElement>(
      `.inputs .gain`,
    )) {
      gainSlider.querySelector<HTMLInputElement>(
        `input[type=range]`,
      )!.disabled = !setup;
      gainSlider.querySelector<HTMLDivElement>(`.volume`)!.style.visibility =
        setup ? "visible" : "hidden";
    }

    for (const phantom of document.querySelectorAll<HTMLInputElement>(
      `.inputs .phantom`,
    )) {
      phantom.querySelector<HTMLInputElement>(
        `input[type=checkbox]`,
      )!.disabled = !setup;
  ...
  private async on_alive() {
    ...
    setupModeSelector.value!.onchange = (_) =>
  ...
  private async updateVu(state: MixerState, levels: Levels) {
    for (const [i, db] of levels.smooth.inputs.entries()) {
      ...
      const channel = this.container.querySelector<HTMLDivElement>(
        `.inputs [data-name=${input.name}]`,
      )!;

      channel.querySelector<HTMLMeterElement>(`meter`)!.value = logLinear(db);
      channel.querySelector<HTMLDivElement>(`.meterdb`)!.innerText =
        formatDb(db);
    }
    for (const [i, db] of levels.smooth.buses.entries()) {
      const output = state.buses[i];
      const bus = this.container.querySelector<HTMLDivElement>(
        `.outputs [data-name=${output.name}]`,
      )!;
  ...
  private async updateState(state: MixerState) {
    for (const channel of state.channels) {
      const input = this.container.querySelector<HTMLDivElement>(
        `.inputs [data-name=${channel.name}]`,
      )!;
      input.querySelector<HTMLInputElement>(`input[type=range]`)!.value =
        channel.gain.toString();
      input.querySelector<HTMLDivElement>(`.gaindb`)!.innerText = formatDb(
        channel.gain,
      );

      input.querySelector<HTMLInputElement>(
        `.phantom input[type=checkbox]`,
      )!.checked = channel.phantom;
    }

    for (const bus of state.buses) {
      const output = this.container.querySelector<HTMLDivElement>(
        `.outputs [data-name=${bus.name}]`,
      )!;
      output.querySelector<HTMLInputElement>(`input[type=range]`)!.value =
        bus.volume.toString();
      output.querySelector<HTMLDivElement>(`.gaindb`)!.innerText = formatDb(
        bus.volume,
      );
    }
    ...
    for (const ch of state.channels) {
      for (const [j, bus] of state.buses.entries()) {
        const send = this.container.querySelector<HTMLDivElement>(
          `.inputs [data-from=${ch.name}][data-to=${bus.name}]`,
        )!;

        send.querySelector<HTMLInputElement>(`input[type=range]`)!.value =
          ch.sends[j].volume.toString();
        send.querySelector<HTMLInputElement>(`.gaindb`)!.innerText = formatDb(
          ch.sends[j].volume,
        );
        send.querySelector<HTMLInputElement>(
          `input.mute[type=checkbox]`,
        )!.checked = ch.sends[j].unmuted;
      }
    }
  }
      ...
      ...state.channels
        ...
        .map(([i, inp]) => (
          <Input
            ...
            onSendMute={(bus) => (muted) =>
              this.client.set_matrix_send(i, bus, !muted)
            }
            onSendVolume={(bus) => (volume) =>

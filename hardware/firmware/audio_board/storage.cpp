...
void eeprom_save_gains(float gains[BUSES][CHANNELS]) {
    eeprom_update_block(gains, (uint8_t *)EEPROM_MATRIX_OFFSET,
                        sizeof(float) * BUSES * CHANNELS);
}

void eeprom_load_gains(float gains[BUSES][CHANNELS]) {
    eeprom_read_block(gains, (uint8_t *)EEPROM_MATRIX_OFFSET,
                      sizeof(float) * BUSES * CHANNELS);
}
...
void eeprom_save_channel_multipliers(float multipliers[CHANNELS]) {
    eeprom_update_block(multipliers,
                        (uint8_t *)EEPROM_CHANNEL_MULTIPLIER_OFFSET,
                        sizeof(float) * CHANNELS);
}
void eeprom_load_channel_multipliers(float multipliers[CHANNELS]) {
    eeprom_read_block(multipliers, (uint8_t *)EEPROM_CHANNEL_MULTIPLIER_OFFSET,
                      sizeof(float) * CHANNELS);
}

void eeprom_save_bus_multipliers(float multipliers[BUSES]) {
    eeprom_update_block(multipliers, (uint8_t *)EEPROM_BUS_MULTIPLIER_OFFSET,
                        sizeof(float) * BUSES);
}
void eeprom_load_bus_multipliers(float multipliers[BUSES]) {
    eeprom_read_block(multipliers, (uint8_t *)EEPROM_BUS_MULTIPLIER_OFFSET,
                      sizeof(float) * BUSES);
}

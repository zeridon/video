...
// defined in teensyaudio_generated.cpp which is generated by helpers/generate_graph.py
extern InputChannel route_inputs[CHANNELS];
extern OutputChannel route_outputs[BUSES];
extern AsyncAudioInputSPDIF3  spdif1;
...
bool set_blob(uint8_t index, const char* text) {
	return blobs[index].set(text);
}

const char* get_blob(uint8_t index) {
	return blobs[index].get();
}

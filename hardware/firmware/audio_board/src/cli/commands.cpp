...
const Cli::CmdDescr Cli::cmds[Cli::num_cmds + 1] = {
	...
	{.name = "channel.labels", .help = "returns a list of the short labels of inputs and buses", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 cli->prefix_ok();
	...
	{.name = "channel.names", .help = "returns a list of the names of inputs and buses", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 cli->prefix_ok();
		 cli->port->print("channels:");
		 for (uint8_t i = 0; i < CHANNELS; i++) {
			 cli->port->print(" ");
	...
	{.name = "levels", .help = "for each input channel and then for each output bus, returns 3 numbers that are the rms, peak and smooth levels", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 Levels& levels = audio_get_levels();
		 cli->prefix_ok();
		 for (uint8_t i = 0; i < CHANNELS + BUSES; i++) {
			 cli->port->print(" ");
	...
	{.name = "matrix", .help = "for each input channel x outpub bus combination, outputs an item in the form <send>*<volume>, where send is 0/1 and volume is a float", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 cli->prefix_ok();
	...
	{.name = "gains", .help = "for each input channel, returns its gain in db", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 cli->prefix_ok();
		 for (uint8_t chan = 0; chan < CHANNELS; chan++) {
			 cli->port->print(" ");
	...
	{.name = "phantoms", .help = "for each input channel, returns its phantom power on/off status (0/1)", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 cli->prefix_ok();
		 for (uint8_t chan = 0; chan < CHANNELS; chan++) {
	...
	{.name = "bus-volumes", .help = "for each output bus, returns its volume", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 cli->prefix_ok();
		 for (uint8_t bus = 0; bus < BUSES; bus++) {
			 cli->port->print(" ");
	...
	{.name = "send.set", .help = "for the given channel/bus crosspoint, set the send bit in the matrix", .arghelp = "<in channel no> <out bus no> (0|1)", .num_args = 3, .callback = [](Cli* cli) {
	...
	{.name = "phantom.set", .help = "for the given input channel, set the phantom power on/off bit", .arghelp = "<in channel no> (0|1)", .num_args = 2, .callback = [](Cli* cli) {
		 uint16_t chan         = cli->hop_uint();
	...
	{.name = "in-gain.set", .help = "for the given input channel, set the input gain in decibels", .arghelp = "<in channel no> <gain>", .num_args = 2, .callback = [](Cli* cli) {
		 uint16_t chan = cli->hop_uint();
	...
	{.name = "bus-volume.set", .help = "for the given out bus, set its global volume to the given value", .arghelp = "<out bus no> <volume>", .num_args = 2, .callback = [](Cli* cli) {
		 uint16_t bus = cli->hop_uint();
		 float    vol = cli->hop_float();
	...
	{.name = "factory-reset", .help = "clear all settings and state stored in the EEPROM", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 audio_reset_default_state();
		 audio_eeprom_save_all();
		 cli->report_ok();
	 }},
	{.name = "commands", .help = "get a list of available commands", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {
		 cli->prefix_ok();
	...
	{.name = "help", .help = "I suppose you think that was terribly clever.", .arghelp = "[command]", .num_args = -1, .callback = [](Cli* cli) {
		 if (cli->cmd[0] == '\0') {
			 cli->prefix_ok();
	...
	{.name = "dbgtest", .help = "print some log messages to check your debug logging", .arghelp = "", .num_args = 0, .callback = [](Cli* cli) {

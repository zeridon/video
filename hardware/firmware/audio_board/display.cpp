...
void draw_meter(ST7735_t3 *display, int16_t x, int16_t y, int16_t w, int16_t h,
                float level) {
    ...
    display->fillRect(x, y + h - yellow_thresh - yfill, w, yfill,
                      ST7735_YELLOW);
...
void draw_channel(ST7735_t3 *display, int rms, int id, ChanInfo *channel_info) {
    ...
    draw_meter(display, 6 + (12 * (id % 6)), offset + 1, 10,
               (SCREEN_HEIGHT / 2) - 13, DbtoLevel(rmsToDb(rms)));
}

...
for day in 1 2; do
	for room in `grep ^1- plaintext|cut -d : -f 1|cut -d - -f 2`; do
		...
		pandoc -f markdown -t latex --variable papersize=a4paper  --variable classoption=twocolumn  --variable columnsep=1in -V geometry:margin=2cm - -o $OUTDIR/$day-$room.pdf
	done
done

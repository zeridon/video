...
You should be a team of at least three per building: AW, H, K, U respectively. J has a dedicated knowledgeable volunteer.

Video coordination happens through the following channels:

\begin{itemize}
  \item Fixed telephony - phone number 200 calls the NOC.
  \item Fixed telephony - phone number 2XX calls the closest VOC.
  \item Fixed telephony - you'll have a phone directory for with phones for all rooms and VOCs.
  \item Matrix - \#video-backstage:fosdem.org is the general channel for announcements and general issues.
  \item Matrix - \#video:fosdem.org is the general channel for visitors, online viewers, etc. for reporting issues and asking questions.
  \item Matrix - there will be separate per-building Matrix rooms for discusssions about the building, sound checks, etc..
  \item Matrix - there are the standard channels for devroom-managers, volunteers and volunteers-backstage, where people also report issues.
\end{itemize}

For invites to all to the matrix channels, please contact the NOC people to invite you.
...
\begin{itemize}
  \item AW: AW.1125
  \item K: Floor 4 between K.4401 and K.4201
  \item H: room H.2111
...
\begin{itemize}
  \item monitor video and update scene if necessary. See the ``Monitoring and control of video streams'' section for more details;
  \item monitor matrix channels for problem reports;
  \item answer the phone in the VOC for bug/problem reports;
  \item communicate with dispatch and NOC/VOCs about room-related issues - closing/opening rooms, etc..
\end{itemize}
...
\begin{itemize}
  ...
  \item Control interface for the video mix, to select the scene to be shown, at \\https://control.video.fosdem.org/ with user and password. Request access from NOC or your per-building video resposible person;
...
The \texttt{showbuilding.php} looks like this:

\includegraphics[width = 170mm]{showbuilding.png}
...
\begin{itemize}
  \item Room name. Its background is while if the sound levels are ok, red if there is no data, and yellow if the audio levels are not within the defined boundaries;
  \item Snapshot of the camera input, updated every 2-3 seconds;
  \item Snapshot of the slides/laptop input, updated every 2-3 seconds;
  \item Snapshot of the mixed stream output from the video mixer, updated every 2-3 seconds. This is what gets sent to the viewers of the streams;
  \item Graph of the levels of the sound. The blue line is the microphone input, the green-yellow is the room ambiance level picked from the camera internal microphone.
\end{itemize}

The three bands on the sound levels are, from top to bottom:

\begin{itemize}
  \item Yellow band, from 0 dbm to -14 dbm - peaking/too high;
  \item Green band, from -14 dbm to -30 dbm - ok;
  \item Red band, below -30 dbm - too quiet or missing;
\end{itemize}
...
\subsection{Per-room control}

The per-room control looks like this:

\includegraphics[width = 170mm]{control.video.png}

The "room status" on top is used to set if the room is either full or not. It's
mostly for use for the devroom managers.

The 4 buttons below control the video mix:

\begin{itemize}
  \item side-by-side presenter - picture in picture, large slides, small camera view. This is the default
  \item side-by-side slides - picture in picture, large camera, small sldies videw.
  \item fullscreen presenter - the camera view on full screen.
  \item fullscreen slides - the slides view on full screen.
\end{itemize}

On the right side there is a preview of the current streams, same as in
\texttt{showbuilding.php} above. Below that is a graph of the output levels,
...
The inputs are the three input on the box, and \texttt{PC} is the analog input
from the HDMI capture card (the sound channel split from it).  It carries
either the camera audio (for a cambox) or the presenter's audio from their HDMI
output (for the slides box).
...
For the large rooms, we use the room's PA and out own microphones.
...
\include{FOSDEM_Room_setup_checklist}
...
\begin{itemize}
  ...
  \item XLR cable from the OUT 2 channel of the audio mixer of the presenter's videobox to our loudspeaker
...
microphone. They use the Sony HXR-NX100 camera. For the rooms that don't have a
PA, there's also a dedicated loudspeaker.
...
\begin{itemize}
  ...
  \item slides boxes (the one for the speaker) only:
    \begin{itemize}
      \item The right HDMI connection should be connected to the projector (HDMI loop out)
      \item The left HDMI connection should have a loose HDMI cable, for connecting to the presenter laptop (HDMI in)
    \end{itemize}
  \item Cam boxes only:
    \begin{itemize}
...
\emph{Do not attempt to power off the box unassisted, as this may damage the
integrity of the system.} If you suspect it may be malfunctioning and there is
not enough diagnostic information, contact the control and monitoring team (see
chapter ``During the event'' for full details). They may already be aware of
the problem, but quick reporting helps cut down on reaction times.

While the box contains a network switch (all four ports are connected),
\emph{do not connect your own equipment} to the box. The boxes are on a special
subnet with reserved bandwidth for the video streaming, and any other equipment
may compromise the quality of the broadcast.

\emph{The boxes have only HDMI input and HDMI output.} For lecturers with only
VGA outputs, the per-building video team is required to ask them for their time
...
to 1080p (1920x1080) at 50Hz. Matching this on the presenter laptop is a good
idea, but not a requirement.

\emph{The display on the box will show all vital information about the current
status of the box.} It shows a snapshot of the stream video, replaced with
signal and its network status (connected/disconnected), the recording status
(yes/no), the streaming status (yes/no) and others every few seconds. The sides
of the display show the audio levels of the built-in audio mixer.

The boxes report all this information to control while their network status is
good, where the control and monitoring team keeps on eye on all rooms. If
...
\section{Tripod} We use the Manfrotto MVH 502. This should hopefully be
self-explanatory. Please offer feedback to VOC if that is not the case.

\clearpage

\section{Camera}
FOSDEM will uses one camera model: the Sony HXR-NX100.
...
\subsection{Camera setup for a room}
...
video team member nearby, contact them
%through the \texttt{\#video:fosdem.org} channel on Matrix https://chat.fosdem.org/\#/room/\#video:fosdem.org or
in \texttt{FOSDEM Video}. If your video box shows issues on the LCD, most
likely somebody is already on their way to you. When communicating with video
team members, please mention the \emph{room number} as opposed to the devroom
name. Devrooms move around, room numbers stay constant.
...
\texttt{sproxy} is a small piece of software available in
https://github.com/FOSDEM/sproxy and FOSDEM's Debian package repository. It's
written in C and its role is to receive a stream of bytes on \texttt{stdin} in
a ring-buffer and provide the following TCP endpoints (up to 32) to clients:
...
\begin{itemize}
  ...
  \item A Radxa X4 board, which controls everything (a normal x86\_64 single-board computer);
...
\begin{itemize}
  ...
  \item A control interface for every room for changing the current video mix, controlling the audio mixers and room status (\texttt{vocto.php}). It's authenticated with users/passwords provided to devroom managers and VOC people;
...
\subsection{video-audio-parser}

Implemented in role \texttt{video-audio-parser}.

The following tasks are handled by the video-audio-parser:

\begin{itemize}
  \item An InfluxDB database with sould levels for voctops and audio mixers (caller \texttt{ebur2})
  \item Instances of \texttt{audiofetcher} that fetch data from the voctops to plot sound levels;
\end{itemize}
...
\begin{itemize}
  ...
  \item 20-minute dumps of video streams, to be processed by \texttt{SReview}
\end{itemize}

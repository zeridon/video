...
\subsection{Per building teams: AW, H, K, U}
Two people per building. One team for AW, H, K and U respectively. J (probably) has a dedicated knowledgeable volunteer.

Coordination with video operations control (VOC) happens through walkies and irc://irc.freenode.net\#fosdem-video.

At the start of the event, these people take the equipment to the rooms. Every team works from the biggest to the smallest room in its building. They do basic setup (cam, sound, plugging in cables, basic volume checks with microphones, ...). They will ensure everything is ready before the first talk starts in each room. 

During the day, they have a spot in the building:
\begin{itemize}
...
They monitor video from the local building and fix local building problems as needed.
...
Their task is to constantly monitor all the equipment and streams for problems, and to arrange fixes for any that occur. They have visual monitoring of all audio streams.

This team does not run out to fix problems themselves. Maintaining contact between everyone is their main priority.

\subsection{In the devroom}
If there is a dedicated person for a devroom, this person:
\begin{itemize}
  \item stays near the video equipment, keeping it safe
  \item makes sure the camera is aimed at the speaker.
  \item monitors the devroom video/audio feeds (headphone!) for problems
  \item signals problems to VOC by irc
\end{itemize}

The devroom volunteer does \emph{NOT} disconnect or turn off any equipment themselves. That is the task of the per building team only.
